document.addEventListener("DOMContentLoaded", () => {
    ...
    Object.entries(charts).forEach(([id, rows]) => {
        ...
        new Chart(canvas, {
            ...
            data: {labels: rows.map(row => row.label), datasets: [{label: "Cards / copies", data: rows.map(row => row.quantity || row.cards || 0), backgroundColor: ["#0d6efd", "#198754", "#ffc107", "#dc3545", "#6f42c1", "#0dcaf0", "#fd7e14", "#20c997"]}]},
